...
class Gridworld(BaseMetaEnv):
    ...
    def get_textual_description(self):
        action_desc_listing = '\n'.join([f"- {action}: {description}" for action, description in dict_actions.items()])
        return f"""
The Gridworld environment is a 2D grid in which the agent can move in 4 directions (up, down, left, right).
...
- other fields can be added depending on the task. This will be specified in the task description.
    ...
    def reset(self, seed : Union[int, None] = None) -> Tuple[
        Observation,
        str,
        Dict[str, Any]
    ]:
        ...
        self.agent_pos = random.choice([0, self.height - 1]), random.choice([0, self.width - 1])
        ...
        if self.name_task == "go_to_center":
            task_description = f"Go to the center of the grid, e.g. at coordinates ({self.height // 2}, {self.width // 2})."
        # Initialize rendering
    ...
    def step(self, action: str) -> Tuple[
        ...
        if action == "UP":
            ...
            self.agent_pos = min(self.height - 1, self.agent_pos[0] + 1), self.agent_pos[1]
        elif action == "LEFT":
            self.agent_pos = self.agent_pos[0], max(0, self.agent_pos[1] - 1)
        elif action == "RIGHT":
            self.agent_pos = self.agent_pos[0], min(self.width - 1, self.agent_pos[1] + 1)
        else:
