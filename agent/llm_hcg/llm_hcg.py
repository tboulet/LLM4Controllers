...
class LLMBasedHierarchicalControllerGenerator(BaseAgent):
    ...
    def __init__(self, config: Dict):
        ...
        self.knowledge_base = KnowledgeBase(config_agent=config)
    ...
    def get_controller(self, task: TaskRepresentation) -> Controller:
        ...
        for no_attempt in range(self.num_attempts_sc):
            ...
            if sc_code is None:
                ...
                self.llm.add_prompt("I'm sorry, extracting the code from your answer failed. Please try again and make sure the code obeys the following format:\n```python\n<your code here>\n```")
            ...
            try:
                ...
                self.llm.add_answer(answer)
                self.llm.add_prompt(f"I'm sorry, an error occured while executing your code. Please try again and make sure the code is correct. Full error info : {full_error_info}")
                if self.config["config_debug"]["breakpoint_on_failed_sc_extraction"]:
                    input("Continue ? ...")
                continue
