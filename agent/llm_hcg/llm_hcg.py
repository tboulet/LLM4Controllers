...
from agent.llm_hcg.knowledge_base import ControllerLibrary
...
class LLMBasedHierarchicalControllerGenerator(BaseAgent):
    ...
    def __init__(self, config: Dict):
        ...
        self.knowledge_base = ControllerLibrary(
        ...
        for no_attempt in range(self.num_attempts_sc):
            ...
            if sc_code is None:
                ...
                if self.config["config_debug"]["breakpoint_on_failed_sc_extraction"]:
                    ...
                    breakpoint()
                continue
            # Extract the imported controllers and the code without the imports
            # list_pc_imports, sc_code = self.extract_pc_imports_and_code_without_it(sc_code)
            # Execute the controller code and retrieve the controller instance
            try:
                self.exec_import_in_namespace(sc_code)
    ...
    def exec_import_in_namespace(self, code: str):
    ...
    def extract_pc_imports_and_code_without_it(self, code: str):
        # Regular expression to capture import statement and controller names
        import_pattern = r"from controllers import (.*)"
        match = re.search(import_pattern, code)

        if match:
            # Extract the controller names from the import statement
            controllers_line = match.group(1)
            controllers = [ctrl.strip() for ctrl in controllers_line.split(",")]

            # Remove the import line from the code
            code_without_import = re.sub(import_pattern, "", code).strip()

            return controllers, code_without_import
        else:
            return [], code
